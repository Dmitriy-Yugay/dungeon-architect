...
class PostWaveExplanationViewTest {
    ...
    fun `victory explains that every hero was defeated before arrival`() {
        ...
        assertEquals("VICTORY - Heart secured", view.headline)
        assertEquals(
            "All 4 heroes were defeated before reaching the heart.",
            view.explanation,
        )
        assertEquals("Heroes: 4 defeated, 0 arrived", view.heroResultLabel)
        assertEquals("Heart health: 10", view.heartLabel)
    ...
    @Test
    fun `victory explains when arrivals occurred but the heart survived`() {
        val view = PostWaveExplanationView.from(
            report(
                outcome = WaveOutcome.VICTORY,
                heartHealth = 6,
        ...
        assertEquals(
            "The heart survived with 6 health after 1 hero arrived; " +
    ...
    fun `defeat explains arrival damage including zero trap metrics`() {
        ...
        assertEquals(
            PostWaveExplanationView(
                headline = "DEFEAT - Heart destroyed",
                explanation =
                    "1 hero reached the heart and reduced its health to 0; " +
                        "0 heroes were defeated.",
                heroResultLabel = "Heroes: 0 defeated, 1 arrived",
                heartLabel = "Heart health: 0",
